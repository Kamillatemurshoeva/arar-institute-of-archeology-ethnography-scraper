...
START_URL = "https://arar.sci.am/dlibra/results?q=&action=SimpleSearchAction&type=-6&p=0&qf1=collections:17"
LIST_URL = "https://arar.sci.am/dlibra/results?q=&action=SimpleSearchAction&type=-6&p={}&qf1=collections:17"
...
def parse_item(session, url):
    ...
    subjects = pick_field(
        fields,
        ["Subject and keywords", "Subjects", "Keywords", "Թեմա և հիմնաբառեր", "Ключевые слова"]
    )
...
def main():
    ...
    for i, url in enumerate(tqdm(links, desc="Scraping"), start=1):
        ...
        if i % 25 == 0:
            save_csv(records, "arar_collection_17_partial.csv")
            save_jsonl(records, "arar_collection_17_partial.jsonl")

    save_csv(records, "arar_collection_17.csv")
    save_jsonl(records, "arar_collection_17.jsonl")
